import numpy as np
import pandas as pd
import math
import matplotlib.pyplot as plt
import datetime
from sklearn import preprocessing

data_raw = pd.read_csv('data_train.csv', index_col=0)

#%% City range
X_CITY_MIN = 3750901.5068
X_CITY_MAX = 3770901.5068
Y_CITY_MIN = -19268905.6133
Y_CITY_MAX = -19208905.6133
X_MAX = 3777099.2656833767
X_MIN = 3740998.35481912
Y_MAX = -19382914.9809002
Y_MIN = -19042656.658487003

#%% Functions
def normalize_x(x):
    return (x - X_MIN )/(X_MAX - X_MIN)

def normalize_y(y):
    return (y - Y_MIN )/(Y_MAX - Y_MIN)

def time_to_range(x):
    if datetime.time(0, 0, 0) < x <= datetime.time(0, 30, 0):
        return 1
    elif datetime.time(0, 30, 0) < x <= datetime.time(1, 0, 0):
        return 2
    elif datetime.time(1, 0, 0) < x <= datetime.time(1, 30, 0):
        return 3
    elif datetime.time(1, 30, 0) < x <= datetime.time(2, 0, 0):
        return 4
    elif datetime.time(2, 0, 0) < x <= datetime.time(2, 30, 0):
        return 5
    elif datetime.time(2, 30, 0) < x <= datetime.time(3, 0, 0):
        return 6
    elif datetime.time(3, 0, 0) < x <= datetime.time(3, 30, 0):
        return 7
    elif datetime.time(3, 30, 0) < x <= datetime.time(4, 0, 0):
        return 8
    elif datetime.time(4, 0, 0) < x <= datetime.time(4, 30, 0):
        return 9
    elif datetime.time(4, 30, 0) < x <= datetime.time(5, 0, 0):
        return 10
    elif datetime.time(5, 0, 0) < x <= datetime.time(5, 30, 0):
        return 11
    elif datetime.time(5, 30, 0) < x <= datetime.time(6, 0, 0):
        return 12
    elif datetime.time(6, 0, 0) < x <= datetime.time(6, 30, 0):
        return 13
    elif datetime.time(6, 30, 0) < x <= datetime.time(7, 0, 0):
        return 14
    elif datetime.time(7, 0, 0) < x <= datetime.time(7, 30, 0):
        return 15
    elif datetime.time(7, 30, 0) < x <= datetime.time(8, 0, 0):
        return 16
    elif datetime.time(8, 0, 0) < x <= datetime.time(8, 30, 0):
        return 17
    elif datetime.time(8, 30, 0) < x <= datetime.time(9, 0, 0):
        return 18
    elif datetime.time(9, 0, 0) < x <= datetime.time(9, 30, 0):
        return 19
    elif datetime.time(9, 30, 0) < x <= datetime.time(10, 0, 0):
        return 20
    elif datetime.time(10, 0, 0) < x <= datetime.time(10, 30, 0):
        return 21
    elif datetime.time(10, 30, 0) < x <= datetime.time(11, 0, 0):
        return 22
    elif datetime.time(11, 0, 0) < x <= datetime.time(11, 30, 0):
        return 23
    elif datetime.time(11, 30, 0) < x <= datetime.time(12, 0, 0):
        return 24
    elif datetime.time(12, 0, 0) < x <= datetime.time(12, 30, 0):
        return 25
    elif datetime.time(12, 30, 0) < x <= datetime.time(13, 0, 0):
        return 26
    elif datetime.time(13, 0, 0) < x <= datetime.time(13, 30, 0):
        return 27
    elif datetime.time(13, 30, 0) < x <= datetime.time(14, 0, 0):
        return 28
    elif datetime.time(14, 0, 0) < x <= datetime.time(14, 30, 0):
        return 29
    elif datetime.time(14, 30, 0) < x <= datetime.time(15, 0, 0):
        return 30
    elif datetime.time(15, 0, 0) < x <= datetime.time(15, 30, 0):
        return 31
    elif datetime.time(15, 30, 0) < x <= datetime.time(16, 0, 0):
        return 32
    else:
        return 0

def check_exit(row):
    if X_CITY_MIN <= row['x_exit'] <= X_CITY_MAX and Y_CITY_MIN <= row['y_exit'] <= Y_CITY_MAX:
        return 1
    else:
        return 0

def check_entry(row):
    if X_CITY_MIN <= row['x_entry'] <= X_CITY_MAX and Y_CITY_MIN <= row['y_entry'] <= Y_CITY_MAX:
        return 1
    else:
        return 0

def normalize_time(t):
    return (t-1)/(32-1)

#%% Find one row only moving devices
# Basically all of the data is at ard 15:00
freq = data_raw['hash'].value_counts()
data_one_row = data_raw[data_raw['hash'].isin(np.array(freq[freq.values==1].keys()))].copy()
data_one_row['city_center'] = data_one_row.apply (lambda row: check_exit(row), axis=1)
data_one_row['time_entry'] = pd.to_datetime(data_one_row['time_entry'], format="%H:%M:%S").dt.time
data_one_row['time_exit'] = pd.to_datetime(data_one_row['time_exit'], format="%H:%M:%S").dt.time
data_one_row['time_range_entry'] = data_one_row['time_entry'].apply(time_to_range)
data_one_row['time_range_exit'] = data_one_row['time_exit'].apply(time_to_range)
data_one_row['time_range_entry'] = normalize_time(data_one_row['time_range_entry'])
data_one_row['time_range_exit'] = normalize_time(data_one_row['time_range_exit'])
data_one_row['x_entry']=normalize_x(data_one_row['x_entry'])
data_one_row['y_entry']=normalize_y(data_one_row['y_entry'])
data_one_row['x_exit']=normalize_x(data_one_row['x_exit'])
data_one_row['y_exit']=normalize_y(data_one_row['y_exit'])
data_one_row.shape[0]

#%% Extract Last and prev position for each devices:
data_with_prev = data_raw[~data_raw['hash'].isin(np.array(data_one_row['hash']))].copy()
data_last_position = data_with_prev.drop_duplicates(subset='hash', keep='last', inplace=False)
data_prev_position = data_with_prev.drop(data_last_position.index)
data_prev_position = data_prev_position.drop_duplicates(subset='hash', keep='last', inplace=False)

data_last_position['time_entry'] = pd.to_datetime(data_last_position['time_entry'], format="%H:%M:%S").dt.time
data_last_position['time_exit'] = pd.to_datetime(data_last_position['time_exit'], format="%H:%M:%S").dt.time
data_last_position['time_range_entry'] = data_last_position['time_entry'].apply(time_to_range)
data_last_position['time_range_exit'] = data_last_position['time_exit'].apply(time_to_range)
data_last_position['time_range_entry']=normalize_time(data_last_position[['time_range_entry']].values)
data_last_position['time_range_exit']=normalize_time(data_last_position[['time_range_exit']].values)
data_last_position['city_center'] = data_last_position.apply (lambda row: check_exit(row), axis=1)

data_prev_position['time_entry'] = pd.to_datetime(data_prev_position['time_entry'], format="%H:%M:%S").dt.time
data_prev_position['time_exit'] = pd.to_datetime(data_prev_position['time_exit'], format="%H:%M:%S").dt.time
data_prev_position['time_range_entry'] = data_prev_position['time_entry'].apply(time_to_range)
data_prev_position['time_range_exit'] = data_prev_position['time_exit'].apply(time_to_range)
data_prev_position['time_range_entry']=normalize_time(data_prev_position[['time_range_entry']].values)
data_prev_position['time_range_exit']=normalize_time(data_prev_position[['time_range_exit']].values)
data_prev_position['city_center_entry'] = data_prev_position.apply (lambda row: check_entry(row), axis=1)
data_prev_position['city_center'] = data_prev_position.apply (lambda row: check_exit(row), axis=1)
data_prev_position['x_entry']=normalize_x(data_prev_position[['x_entry']].values)
data_prev_position['y_entry']=normalize_y(data_prev_position[['y_entry']].values)
data_prev_position['x_exit']=normalize_x(data_prev_position[['x_exit']].values)
data_prev_position['y_exit']=normalize_y(data_prev_position[['y_exit']].values)
data_prev_position['exitPoint_x'] = normalize_x(data_last_position[['x_entry']].values)
data_prev_position['exitPoint_y'] = normalize_y(data_last_position[['y_entry']].values)
data_prev_position['exitPoint_time_range_entry'] = np.array(data_last_position[['time_range_entry']])
data_prev_position['exitPoint_time_range_exit'] = np.array(data_last_position[['time_range_exit']])
data_prev_position = data_prev_position.drop(['hash','trajectory_id', 'vmax', 'vmin','vmean', 'time_entry', 'time_exit'], axis=1)
data_prev_position.head()

#%% output
data_one_row = data_one_row.drop(['hash','trajectory_id', 'vmax', 'vmin','vmean', 'time_entry', 'time_exit', 'x_exit', 'y_exit'], axis=1)
data_one_row.to_csv('oneRow_training.csv')
data_last_position.to_csv('last_position_with_timeRangeAndCityCentered.csv')
data_prev_position.to_csv('prev_position_with_timeRangeAndCityCentered.csv')
data_with_prev.to_csv('data_train_wo_oneRow.csv')

data_prev_position.min()
